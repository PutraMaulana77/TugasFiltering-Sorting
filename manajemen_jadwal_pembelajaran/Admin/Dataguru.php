...
<head>
  ...
  <style>
    ...
    @media print {
      ...
      .navbar, .btn, .d-flex {
        display: none; /* Sembunyikan navbar, tombol, dan form pencarian saat mencetak */
      }
    }
...
<!-- Button -->
    ...
    <?php if(isset($_GET['cari'])): ?>
      <a href="Dataguru.php" class="btn btn-danger ml-2">X</a>
    <?php endif; ?>
  </form>
</div>
...
<div class="container mt-3">
  <form action="" method="get" class="d-flex">
    ...
    <!-- TOMBOL PRINT/CETAK -->
    <a href="cetak.php" target="_blank" class="btn btn-primary ml-2">Cetak</a>
  </form>
</div>
...
<script>
  ...
  <table id="myTable" class="table table-striped table-bordered mt-1">
    ...
    if(isset($_GET['cari'])){
      $cari = $_GET['cari'];
      $data = mysqli_query($koneksi, "SELECT * FROM guru WHERE NUPTK LIKE '%$cari%' OR Nama_Guru LIKE '%$cari%' OR Bidang_Mengajar LIKE '%$cari%' OR Kontak LIKE '%$cari%'");
    } else {
    ...
    ?>
      <tr>
        ...
        <td><?php echo $d['Bidang_Mengajar']; ?></td>
        <td><?php echo $d['Kontak']; ?></td>
        <td>
          <a href="edit.php?id=<?php echo $d['NUPTK']; ?>" class="btn btn-success btn-sm mr-2">UBAH</a>
          <a href="hapus.php?nuptk=<?php echo $d['NUPTK']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Anda yakin akan menghapus data ini?')">HAPUS</a>
        </td>
      </tr>
    <?php 
    }
